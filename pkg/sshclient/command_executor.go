...
func NewCommandExecutor(connManager *ConnectionManager, options CommandExecutorOptions) (*CommandExecutor, error) {
	...
	_, err = commExecutor.readOutputUntilPrompt([]string{"MA5683T>", "MA5800-X7>"})
	if err != nil {
		return nil, err
	}
...
func (c *CommandExecutor) ExecuteCommand(command string, prompt []string) (string, error) {
...
func (c *CommandExecutor) GetUnmanagedOpticalNetworkTerminals() ([]UnmanagedONT, error) {
	...
	output, err := c.ExecuteCommand("display ont autofind all", []string{"MA5683T(config)#", "MA5800-X7(config)#"})
	if err != nil {
...
func (c *CommandExecutor) GetOpticalInfo(port, ontID int) (*OpticalInfo, error) {
	...
	output, err := c.ExecuteCommand(fmt.Sprintf("display ont optical-info %d %d", port, ontID), []string{
		fmt.Sprintf("MA5683T(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot),
		fmt.Sprintf("MA5800-X7(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot)},
	)
	if err != nil {
		return nil, fmt.Errorf("failed to run command: %v", err)
	}
...
func (c *CommandExecutor) GetGeneralInfoBySn(sn string) (*GeneralInfo, error) {
	...
	output, err := c.ExecuteCommand(fmt.Sprintf("display ont info by-sn %s", strings.Split(sn, " ")[0]), []string{"MA5683T(config)#", "MA5800-X7(config)#"})
	if err != nil {
		return nil, fmt.Errorf("failed to run command: %v", err)
	}
...
func (c *CommandExecutor) EnterInterfaceGPONMode(frame int, slot int) error {
	...
	_, err := c.ExecuteCommand(fmt.Sprintf("interface gpon %d/%d", frame, slot), []string{
		fmt.Sprintf("MA5683T(config-if-gpon-%d/%d)#", frame, slot),
		fmt.Sprintf("MA5800-X7(config-if-gpon-%d/%d)#", frame, slot),
	})
	if err != nil {
...
func (c *CommandExecutor) DeleteOpticalNetworkTerminal(port int) error {
	...
	_, err := c.ExecuteCommand(fmt.Sprintf("ont delete %d all", port), []string{"(y/n)[n]:"})
	if err != nil {
		return fmt.Errorf("failed to run command: %v", err)
	}

	_, err = c.ExecuteCommand("y", []string{
		fmt.Sprintf("MA5683T(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot),
		fmt.Sprintf("MA5800-X7(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot),
	})
	if err != nil {
...
func (c *CommandExecutor) AddNativeVirtualLan(port, ontID int) error {
	...
	output, err := c.ExecuteCommand(fmt.Sprintf("ont port native-vlan %d %d eth 1 vlan 20 priority 0", port, ontID), []string{
		fmt.Sprintf("MA5683T(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot),
		fmt.Sprintf("MA5800-X7(config-if-gpon-%d/%d)#", c.ExecutorContext.Frame, c.ExecutorContext.Slot),
	})
	if err != nil {
...
func (c *CommandExecutor) AddServicePort(vlan, frame, slot, port, ontID int) error {
	...
	output, err := c.ExecuteCommand(
		fmt.Sprintf(
			"service-port vlan %d gpon %d/%d/%d ont %d gemport 20 multi-service user-vlan 20 tag-transform translate inbound traffic-table index 10 outbound traffic-table index 10",
			vlan,
			frame,
			slot,
			port,
...
func (c *CommandExecutor) UndoServicePort(id int) error {
	...
	output, err := c.ExecuteCommand(fmt.Sprintf("undo service-port %d", id), []string{"MA5683T(config)#"})
	if err != nil {
		return fmt.Errorf("failed to run command: %v", err)
	}
...
func (c *CommandExecutor) quit(exit bool) error {
	...
	if c.ExecutorContext.Level >= 3 {
		_, err = c.ExecuteCommand("quit", []string{"MA5683T(config)#"})
		if err != nil {
			return fmt.Errorf("failed to run command: %v", err)
		}
	...
	if c.ExecutorContext.Level >= 2 {
		_, err = c.ExecuteCommand("quit", []string{"MA5683T#"})
		if err != nil {
			return fmt.Errorf("failed to run command: %v", err)
		}
	...
	_, err = c.ExecuteCommand("quit", []string{"before logout"})
	if err != nil {
		return fmt.Errorf("failed to run command: %v", err)
	}

	_, err = c.ExecuteCommand("y", []string{"to log on"})
	if err != nil {
		return fmt.Errorf("failed to run command: %v", err)
	}
...
func (c *CommandExecutor) readOutputUntilPrompt(prompts []string) (string, error) {
...
func (c *CommandExecutor) enable() error {
	...
	_, err := c.ExecuteCommand("enable", []string{"MA5683T#", "MA5800-X7#"})
	if err != nil {
...
func (c *CommandExecutor) config() error {
	...
	_, err := c.ExecuteCommand("config", []string{"MA5683T(config)#", "MA5800-X7(config)#"})
	if err != nil {
